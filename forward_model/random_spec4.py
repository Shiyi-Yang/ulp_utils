...
for i in range(gpu_arr.shape[0]):
    print(i)
    #spec_temp = forward_model_gpu_T.gen_forward_model(f2,T,Ne_arr[i+3*gpu_arr.shape[0]],Te_arr[i+3*gpu_arr.shape[0]],Ti_arr[i+3*gpu_arr.shape[0]],ion_composition)
    spec_gpu = forward_model_gpu_T.gen_forward_model(f2,T,Ne_arr[i+3*gpu_arr.shape[0]],Te_arr[i+3*gpu_arr.shape[0]],Ti_arr[i+3*gpu_arr.shape[0]],ion_composition)
    ...
    spec_c = forward_model_gpu_T.gen_forward_model(f1,T,Ne_arr[i+3*gpu_arr.shape[0]],Te_arr[i+3*gpu_arr.shape[0]],Ti_arr[i+3*gpu_arr.shape[0]],ion_composition)
...
t2 = time.time()
#np.savez(Tefolder+'spec4', gpu_arr = gpu_arr,par_arr = par_arr,amp_arr = amp_arr, mu_arr = mu_arr, time=t2-t1)  
np.savez(Tefolder+'spec4',par_arr = par_arr,amp_arr = amp_arr, mu_arr = mu_arr, time=t2-t1)  
